Here's a README.md of a project:
...
Here's the project's main script, timelapse.py:
----------
import argparse
import datetime
import os
...
Currently the frames it copies over to build a time lapse are one frame every minute (which is the amount of frames that exist on the base directory anyway). I want you to add an optional launch arg called --interval that decides the interval, in minutes, of frames to copy over (default=1). Everything else should behave the same. So for example, if a day's start range is 12:00 and the end range is 13:00 and `--interval 5`, only the photos from 12:00, 12:05, 12:10, 12:15, ... 12:55, 13:00 should be copied over.

I foresee one snag with this when it comes to choosing the thumbnail for the youtube video. Currently it chooses it based on the midpoint minute of the first day's time range. Have it still try to find the midpoint of the first day's time range, but aligned to the nearest still we'd have copied over for that day based on the interval.

Output the new timelapse.py
