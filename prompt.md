...
What is a minimal change I can make to timelapse.py and the readme such that:
- If no `--date` is provided, the default will be the most recent monday _before_ the most recent friday (inclusive of today if it's currently friday). So for example, if today is friday, the default date will be the most recent monday. But if today is Tuesday, the default date will be the monday from 8 days prior, not 1 day prior.
- If no `--days` is provided, the default will be the amount of full days between the start `--date` and today, inclusive of `--date` and today, but with a maximum default of 6 days.
