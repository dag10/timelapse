Make a change to youtube.py so that if just the thumbnail-setting step fails, but it still successfully uploaded in the first place, still add it to the playlist, but warn me that it failed to set the thumbnail. Also, if adding to playlist fails, warn me too. But still if any part of uploading fails don't try the later steps.

Here's the latest youtube.py for reference:
